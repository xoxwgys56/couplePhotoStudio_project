...
public class MemoCalendar extends CalendarDateManager{ // CalendarDataManager의 GUI + 메모기능 + 시계
	// 창 구성요소와 배치도
	JFrame mainFrame;
	...
	JPanel memoPanel;
		...
		JButton delBut; 
		JButton clearBut;
		JButton whoBut;//추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가
	
	JPanel frameBottomPanel;
		JLabel bottomInfo = new JLabel(" 부부사진관  ");
	//상수, 메세지
	...
	public MemoCalendar(){ //구성요소 순으로 정렬되어 있음. 각 판넬 사이에 빈줄로 구별
		...
		try{					//18.05.17 - 김용태 - UIManager.setLookAndFeel의 window버전 사용시  프레임창을 닫은 경우 부모 프레임의 버튼 배경의 색깔이 투명해짐
		...
		memoPanel=new JPanel();
			...
			saveBut.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent arg0) {
					try {
						...
						String memo = memoArea.getText();  //메모내용저장 
						if(memo.length()>0){
			...
			delBut = new JButton("Del");//추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가
			...
			//추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가
			whoBut = new JButton("Who");
			whoBut.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent arg0) {
					ListPanel.dateserch(calYear+"-"+(calMonth+1)+"-"+calDayOfMon);
				}
			});
			//추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가
			...
			memoSubPanel.add(whoBut);//추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가추가
	...
	private class ThreadConrol extends Thread{
		public void run(){
			...
			while(!Thread.currentThread().isInterrupted()){ //인터럽트 발생시만 함. flag응용법
				try{
